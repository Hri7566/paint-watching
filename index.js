(async () => {
            ...
            if (!objs.find(o => o.id == obj.id)) {
                ...
                if (obj.static) {
                    for (const key of Object.keys(obj)) {
                        ObjectHandler.setStaticProperty(obj.id, key, obj[key]);
                    }
                }
            }
        }
    ...
    const defaultLocationTable = [
        {
            ...
            aliases: ["inside", "in"],
            world: "Earth",
            objects: [await ObjectHandler.new("couch")],
            reach: ["outside"]
        },
        {
            id: "outside",
    ...
    db.getOwnParticipant = async () => {
        return await db.get("ownParticipant").catch(err => {
            gName, gColor;
        });
    };
    ...
    class Bot {
        ...
        static bindEventListeners() {
            ...
            this.client.on("a", msg => {
                console.log(
                    `${msg.p._id.substring(0, 6)} ${msg.p.name}: ${msg.a}`
                );
            ...
            this.client.once("wserror", msg => {
                console.warn(
                    "Issues connecting to MPP server, hopefully not a server outage or ban..."
                );
            });
        }
        ...
        static sendUserset(name, color) {
            this.client.sendArray([
                {
                    m: "userset",
                    set: { name: name || gName, color: color || gColor }
                }
            ]);
        }
        ...
        static getUserFuzzy(str) {
            ...
            for (let p of Object.values(this.client.ppl)) {
                return (
                    ...
                    p._id.toLowerCase().includes(str) ||
                    p.id.toLowerCase().includes(str)
                );
            }
        }
    }
    ...
    class CommandHandler {
        ...
        static async handleCommand(msg, bot) {
            ...
            prefixLoop: for (let prefix of this.prefixes) {
                if (!prefix.separated) {
            ...
            cmdLoop: for (let cmd of this.commands) {
                let cont = false;
    ...
    CommandHandler.addCommand(
        new Command("help", ["help", "h", "cmds"], (msg, say) => {
            // say("help menu goes here");
            let list = CommandHandler.commands
    ...
    CommandHandler.addCommand(
        new Command("go", ["go", "move"], async (msg, say) => {
            ...
            for (let l of loctab) {
                ...
                let alias = false;
                aliasLoop: for (let al of l.aliases) {
                    if (al.includes(go)) {
                        alias = true;
                        break aliasLoop;
                    }
                ...
                if (!l.id.includes(go) && !l.displayName.includes(go) && !alias)
                    continue;
                loc = l;
            }
            ...
            if (!loc) {
                ...
                return noMapAnswers[
                    Math.floor(Math.random() * noMapAnswers.length)
                ];
            }
            ...
            if (loc.banned) {
                if (loc.banned.includes(msg.p._id)) {
                    if (loc.id == cur)
                        await db.setLocation(msg.p._id, defaultLocationId);
                ...
                return db
                    ...
                    .then(res => {
                        if (cont)
                            return `Friend ${
                                msg.p.name
                            } get SUCCESS: ${JSON.stringify(res)}`;
                    });
            },
            true
        )
    ...
    CommandHandler.addCommand(
        new Command(
            ...
            async (msg, say) => {
                let argslice = msg.a
                    .substring(
                        msg.args[0].length + msg.args[1].length + "  ".length
                    )
                    .trim();
                return db
                    .put(msg.args[1], argslice)
                    .then(res => {
                        return `Friend ${
                            msg.p.name
                        } put SUCCESS: ${JSON.stringify(res)}`;
                    })
                    .catch(err => {
                        return (
                            `Friend ${msg.p.name} put FAIL: ${err}`, undefined
                        );
                    });
            },
            true
        ...
        new Command(
            ...
            async (msg, say) => {
                return db
                    ...
                    .catch(err => {
                        return (
                            `Friend ${msg.p.name} del FAIL: ${err}`, undefined
                        );
                    });
            },
            true
    ...
    CommandHandler.addCommand(
        new Command("id", ["id", "myid", "qmyid"], async (msg, say) => {
    ...
    CommandHandler.addCommand(
        new Command(
            ...
            async (msg, say) => {
                ...
                if (!loc)
                    return `Missed location ${qid} (not in location table) :(`;
                loctab[loctab.indexOf(loc)] = defaultLocationTable.find(
                    l => l.id == qid
                );
    ...
    CommandHandler.addCommand(
        new Command("isadmin", ["isadmin"], async (msg, say) => {
            let check = msg.p._id;
    ...
    CommandHandler.addCommand(
        new Command("sit", ["sit"], async (msg, say) => {
            let sitobj;
            let loc = await db.getLocation(msg.p._id);

            for (let obj of loc.objects) {
                if (
                    (await ObjectHandler.getStaticProperty(
                        obj.id,
                        "enableSit"
                    )) == true
                ) {
            ...
            return `Friend ${
                msg.p.name
            } is now sitting on the ${ObjectHandler.getName(sitobj.id)}.`;
        })
    );
})();
